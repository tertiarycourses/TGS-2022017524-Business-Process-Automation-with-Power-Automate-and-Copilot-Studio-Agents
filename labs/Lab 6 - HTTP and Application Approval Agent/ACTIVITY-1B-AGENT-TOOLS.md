# Module 4 Lab 6b — SharePoint as an Agent Tool

The same onboarding decision, built a different way.

**Lab 6** runs the SharePoint lookup as a **step before** the agent. The
lookup always happens; the agent is handed the answer.

**Lab 6b** gives the agent SharePoint as a **tool**. The agent decides for
...
Neither is "better". Lab 6 is what you want for a regulated decision that
must be auditable. Lab 6b is what you want when the task is open-ended and
...
`Module 4 Lab 6b - Onboarding with Agent Tools`.
...
**Leave Filter Query empty.** This is the key difference. In Lab 6 you
...
Rewritten for tool use. Three things changed from Lab 6:
...
Same as Lab 6 — **Custom structured output** with the same JSON Schema.
...
Identical to Lab 6. Copy both from `LAB-GUIDE.md` Part 2, Nodes 4 and 5.
...
Run **TC5** — lowercase `s8412345d` — through Lab 6b several times.

In Lab 6 the `toUpper()` is in the filter expression, so the match is
...
1. Lab 6b needed far more instruction to get the same behaviour. What
   does that tell you about where complexity goes when you make something
   "more agentic"?

2. Lab 6's flow diagram shows the duplicate check. Lab 6b's does
...
4. Lab 6b costs more per run. For 10 applications a day that is nothing.
...
**Use the step approach (Lab 6) when:**
...
**Use the tool approach (Lab 6b) when:**

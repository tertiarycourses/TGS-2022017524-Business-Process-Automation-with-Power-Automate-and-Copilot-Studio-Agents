/* ============================================================================
   ...
   The Copilot Studio / Power Automate HTTP POST URL is entered in the
   "Lab configuration" panel on the page, so you never have to edit this file.
   It is saved to localStorage.

     • HTTP POST URL — shown in the "When a HTTP request is received" trigger,
       after the flow has been PUBLISHED (not merely saved).
...
function renderWebhookStatus() {
  ...
  if (!url) {
    webhookStatus.textContent = "No HTTP POST URL set — the form cannot be submitted.";
    ...
    webhookStatus.textContent = "URL is missing its sig= signature — copy the whole URL from the trigger.";
    webhookStatus.className = "config-status is-warn";
  } else {
    webhookStatus.textContent = "HTTP POST URL detected. The flow must be PUBLISHED, not just saved.";
...
form.addEventListener("submit", async (event) => {
  ...
  if (!endpoint) {
    ...
    errorBox.textContent = "Enter your HTTP POST URL in the Lab configuration panel first.";
    errorBox.hidden = false;
  ...
  // The Copilot Studio HTTP trigger validates against its JSON schema and
  // rejects the request outright on a type mismatch. HTML form controls always
  // yield strings, so the two Yes/No selects must be coerced to real booleans
  // or the flow returns 400 TriggerInputSchemaMismatch. (n8n coerced these
  // silently; Copilot Studio does not.)
  payload.pep = payload.pep === "Yes";
  payload.foreignTaxResident = payload.foreignTaxResident === "Yes";
  ...
  try {
    ...
    // The flow's Response action returns the agent's structured decision.
    ...
    errorBox.textContent =
      ...
      `(Trainer: check the HTTP POST URL in the Lab configuration panel, and that the flow has been PUBLISHED.)`;
    errorBox.hidden = false;
...
function showResult(decision) {
  ...
  const style = DECISIONS[key] || {
    ...
    fallback: "We could not read the decision. Check the flow run history in Copilot Studio (Activity tab)."
  };

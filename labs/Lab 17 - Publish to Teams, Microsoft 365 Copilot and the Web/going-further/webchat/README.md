...
- The **HR Agent is built and Published** — parent and children, per the
  [main HR Agent lab](../index.md). Re-check ⋯ → **Version history**: `LIVE` and `CURRENT DRAFT`
  match.
...
> **Unlike Lab 13's page, this one cannot be double-clicked open.** Lab 13 worked from `file://`
...
| Connects but answers like a stranger ("I can't see your records") | Signed in with an account that has no row in `staff-leave.csv` — the flow's lookup found nothing. Same unnormalised-lookup family as the four traps in the [Lab 5 index](../../index.md) |

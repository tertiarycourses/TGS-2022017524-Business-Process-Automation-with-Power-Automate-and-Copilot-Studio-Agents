/* ============================================================================
   ...
   LU2 Activity 2b — Copilot Studio build

   The widget POSTs the client's concern to a Copilot Studio workflow's HTTP
   trigger. The workflow runs an Agent that classifies the concern, reads the
   emotional tone, raises compliance flags, and DRAFTS a reply — then stops on a
   Human review node.

   The draft is never returned to this page. What comes back is an
   acknowledgement plus a two-field assessment — because the reply itself must be
   approved by a licensed human before the client is allowed to see it. That is
   the whole point of the activity.

   You never edit this file. Paste the workflow URL into the Lab configuration
   panel on the page; it is remembered in localStorage.
   ========================================================================== */
const DEFAULT_WEBHOOK_URL = "";
const STORAGE_KEY = "meridian.copilot.workflowUrl";
...
function renderWebhookStatus() {
  ...
  if (!url) {
    webhookStatus.textContent = "No HTTP POST URL set — the chat cannot send.";
...
form.addEventListener("submit", async (event) => {
  ...
  if (!endpoint) {
    bubble("No workflow URL is configured. Set it in the <strong>Lab configuration</strong> panel on this page.", "msg-error");
  ...
  try {
    ...
    bubble(
      ...
      `<p><em>Trainer: check the workflow URL in the Lab configuration panel, and that the workflow is ` +
      `Published. If the run succeeded in Monitor but this still failed, it is CORS — serve this page from ` +
      `SharePoint rather than localhost.</em></p>`,
      "msg-error"
    );
  } finally {
    setLoading(false);

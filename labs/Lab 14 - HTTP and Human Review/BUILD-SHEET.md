...
Flow: a **new** workflow — `Lab 14 - HTTP and Human Review`
...
> **You cannot copy Lab 13.** The Copilot Studio designer has no *Save As* or
...
Lab 13 was four nodes and the agent talked straight back to the browser. This one
adds everything that exists *because nobody is allowed to trust the agent*:

| | Lab 13 (Investment Advisor) | Lab 14 (this one) |
|---|---|---|
...
Lab 13 needed a FAQ because it answered factual questions about the firm. This
...
Point it at `Handover Queue.xlsx` → the **`Drafts`** table (Task 0 in the
...
> refused the underscore in `HandoverQueue` and stored `HandoverQueue`, so the
...
> there for days. That screen is the deliverable of Lab 14 — a workflow that
...
**Worth adding too:** a `Comments` text input. Right now every `HandoverQueue`
...
**`HandoverQueue`**. Rename it **Assign_To_Human_Agent**.
...
----
The full record is in the HandoverQueue table of Lab 14 - Handover Queue.xlsx.
...
```bash
cd labs/Lab 14 - HTTP and Human Review/website
...
rather than `localhost`. Same wall as Lab 12's alternative build and Lab 2 — see
[`../Lab 12 - HTTP and Application Approval Agent/archive-alternative-builds/ALT-BUILD-gemini-over-http.md`](../Lab 12 - HTTP and Application Approval Agent/archive-alternative-builds/ALT-BUILD-gemini-over-http.md),

...
**Owns priority and routing.** It is the only agent in Lab 6 whose entire job is to make a
...
`Lab 8 - IT Support Agent` → **Connected agents → +** → `Triage Agent` → description:
...
Every other agent in Lab 6 is judged on whether it said something true. This one is judged on

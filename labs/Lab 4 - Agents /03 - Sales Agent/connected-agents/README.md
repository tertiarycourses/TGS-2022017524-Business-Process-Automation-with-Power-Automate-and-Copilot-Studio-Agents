...
`Sales Agent` → **Connected agents → +** → pick the child → write its description → **publish the
...
Ask the class why the *public-facing* agent has the least authority of the four in Lab 4. The
...
**Cases 3, 4 and 5 are the grounding probes**, carried over from Lab 9's test set. They are the

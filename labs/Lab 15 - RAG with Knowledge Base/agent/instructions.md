# Agent node — Instructions (Lab 15 — Knowledge source)
...
[Lab 16](../Lab 16 - RAG with Pinecone/) — that version opens by
...
the Lab 16 version — a pasted `@{triggerBody()?['message']}` is inert text, and the run

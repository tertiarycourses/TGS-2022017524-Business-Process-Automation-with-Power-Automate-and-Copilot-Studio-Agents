# Build sheet — Lab 15: RAG with a Knowledge base

The same chatbot as [Lab 16](../Lab 16 - RAG with Pinecone/index.md), with the entire retrieval half
deleted.

Flow: `Lab 15 - RAG with Knowledge Base`
...
> That absence is the lesson. Lab 16 needed a Python ingestion script, a Pinecone
...
results to return. You chose none of those. In Lab 16 you chose all of them.

### 2b — Instructions

Paste **the Lab 15 version** from [`agent/instructions.md`](agent/instructions.md), then
insert the question with the ⚡ picker: cursor after `Customer question:` →
**⚡ → When a HTTP request is received → message**.

> ### ⚠️ The wording differs from Lab 16, and this is where the day goes
>
> 3b says *"using only the course brochures **provided below**"* — correct there,
> because Compose injects them. Lab 15 injects nothing; the agent must search.
> The line must read:
>
> ```
...
> ```
>
> Leave Lab 16's wording in place and the agent is told its only permitted source is
...
### 2c — If you built this by copying Lab 16

Duplicating the flow carries Lab 16's Instructions verbatim, including its final
chip:
...
That `builtinFunction-…` is Lab 16's Compose node, which does not exist here. The
...
Clear the Instructions box completely before pasting the Lab 15 version.
...
the Lab 15 version carries the fix:
...
are added after generation. **Neither defect occurs in Lab 16**, where retrieved
...
Expect ~25s — slower than Lab 16, because SharePoint retrieval plus generation is a
...
| | Lab 16 — Pinecone | Lab 15 — Knowledge base |
|---|---|---|
| Nodes | 5 | 3 |
...
**That last row is the debrief.** When Lab 15 retrieves the wrong brochure, there
is no chunk size to change, no `top_k` to raise, no embedding model to swap. You
can rewrite the instruction and hope. Lab 16 gives you four levers and four ways
to get it wrong.

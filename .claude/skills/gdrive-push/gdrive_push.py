...
def main():
    ...
    deck_ppt = newest(os.path.join(cw, "*-v*.pptx")) or newest(os.path.join(cw, "*.pptx"))
    if not deck_ppt:
        raise SystemExit(f"No slide deck found in {cw}")
    ...
    assessments = (sorted(glob.glob(os.path.join(repo, "assessment", "*.docx")))
                   or sorted(glob.glob(os.path.join(repo, "assessemnt", "*.docx"))))

---
name: lms-push
description: Push the current courseware's Google Drive links into the course record on lms-tms.tertiaryinfotech.com via its API — sets Trainer Slides URL (PPT from Master Trainer Slides), Learner Slides URL (slide PDF from Learner Guide), Learner Guide URL (LG PDF), Lesson Plan URL (LP PDF) and Activities/Lab URL (the Activities folder). The Drive folder is read from the course's own Courseware Link field via the API. Use when the user runs /lms-push or asks to update/sync the LMS-TMS course links.
---
...
| Activities/Lab URL | `activitiesUrl` | the **Activities** FOLDER itself — the labs are a tree, so this links the folder, not a file. Missing labs are reported, not fatal. |
